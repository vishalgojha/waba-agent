...
function registerClientsCommands(program) {
  ...
  c.command("list")
    ...
    .action(async (_opts, cmd) => {
      ...
      logger.warn("Deprecated path: `waba clients` now routes to `waba ts clients`. Use `waba ts clients ...` directly.");
      try {
        await runTsClientsList({ json });
      } catch (err) {
        logger.warn(`TS clients list unavailable, falling back to legacy clients store: ${err?.message || err}`);
  ...
  c.command("add")
    ...
    .action(async (name, opts) => {
      logger.warn("Deprecated path: `waba clients add` now routes to `waba ts clients add`.");
      try {
        ...
        logger.warn(`TS clients add unavailable, falling back to legacy clients store: ${err?.message || err}`);
  ...
  c.command("switch")
    ...
    .action(async (name) => {
      logger.warn("Deprecated path: `waba clients switch` now routes to `waba ts clients switch`.");
      try {
        await runTsClientsSwitch({ name });
      } catch (err) {
        logger.warn(`TS clients switch unavailable, falling back to legacy clients store: ${err?.message || err}`);
  ...
  c.command("remove")
    ...
    .argument("<name>", "client name")
    .action(async (name) => {
      logger.warn("Deprecated path: `waba clients remove` now routes to `waba ts clients remove`.");
      try {
        await runTsClientsRemove({ name });
      } catch (err) {
        logger.warn(`TS clients remove unavailable, falling back to legacy clients store: ${err?.message || err}`);
