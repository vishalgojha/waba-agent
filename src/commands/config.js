...
const { runTsConfigShow, runTsConfigSet, runTsConfigUnset } = require("./ts");
...
function registerConfigCommands(program) {
  ...
  c.command("show")
    ...
    .action(async (_opts, cmd) => {
      ...
      logger.warn("Deprecated path: `waba config` now routes to `waba ts config`. Use `waba ts config ...` directly.");
      try {
        await runTsConfigShow({ json });
      } catch (err) {
        logger.warn(`TS config show unavailable, falling back to legacy config: ${err?.message || err}`);
        ...
        try {
          ...
          logger.warn(`TS config bridge unavailable, falling back to legacy config: ${bridgeErr?.message || bridgeErr}`);
        }
  ...
  c.command("set")
    ...
    .action(async (key, value, opts) => {
      logger.warn("Deprecated path: `waba config set` now routes to `waba ts config set`.");
      try {
        await runTsConfigSet({ key, value, client: opts.client });
      } catch (err) {
        logger.warn(`TS config set unavailable, falling back to legacy config: ${err?.message || err}`);
  ...
  c.command("unset")
    ...
    .action(async (key, opts) => {
      logger.warn("Deprecated path: `waba config unset` now routes to `waba ts config unset`.");
      try {
        await runTsConfigUnset({ key, client: opts.client });
      } catch (err) {
        logger.warn(`TS config unset unavailable, falling back to legacy config: ${err?.message || err}`);
        const raw = normalizeMultiClient(await readConfigRaw());
        let removed = false;
        if (opts.client) {
          const n = safeClientName(opts.client);
